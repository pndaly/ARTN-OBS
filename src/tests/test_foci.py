...
# -
def test_foci_1():
    """ test Foci() for incorrect input(s) """
    with ptr(Exception):
        Foci(telescope=random.choice([None, get_hash(), {}, [], ()]))


def test_foci_2():
    """ test Foci() for incorrect input(s) """
    with ptr(Exception):
        Foci(instrument=random.choice([None, get_hash(), {}, [], ()]))
...
def test_foci_3():
    ...
    assert isinstance(Foci(_tel, _ins), observations.foci.Foci) in OBS_TRUE_VALUES
...
def test_foci_4():
    ...
    _ins = random.choice(INS__INSTRUMENTS)
    _tel = INS__TELESCOPE[_ins]
    assert isinstance(Foci(_tel, _ins).telescope, telescopes.factory.Telescope) in OBS_TRUE_VALUES
...
def test_foci_5():
    ...
    _ins = random.choice(INS__INSTRUMENTS)
    _tel = INS__TELESCOPE[_ins]
    assert isinstance(Foci(_tel, _ins).instrument, instruments.factory.Instrument) in OBS_TRUE_VALUES


def test_foci_6():
    """ test Foci() returns correct attribute(s) """
    _ins = random.choice(INS__INSTRUMENTS)
    _tel = INS__TELESCOPE[_ins]
    _d = Foci(_tel, _ins)

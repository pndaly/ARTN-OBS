...
def test_dec_from_decimal_1():
    _dec = random.uniform(-89.0, 89.0)
    assert math.isclose(dec_to_decimal(dec_from_decimal(_dec)), _dec, rel_tol=TEST_TOLERANCE['3dp'])
...
def test_dec_to_decimal_2():
    _v = f"{random.randint(-89, 89):02d}:{random.randint(1, 59):02d}:{random.randint(1, 59):02d} degrees"
    assert dec_to_decimal(_v) is not math.nan


def test_dec_to_decimal_3():
    _v = f"{random.randint(-89, 89):02d}:{random.randint(1, 59):02d}:{random.randint(1, 59):02d} degrees"
    assert -90.0 < dec_to_decimal(_v) < 90.0
...
def test_dec_to_dms_0():
    assert all(dec_to_dms(_k) is None for _k in TEST_INVALID_INPUTS[:-2])
...
# -
def test_get_date_0():
    assert re.match(OBS_ISO_PATTERN, get_date()) is not None
...
def test_get_hash_1():
    assert all(get_hash(_k) is None for _k in TEST_INVALID_INPUTS[2:])
...
def test_jd_to_mjd_2():
    ...
    assert math.isclose(
        abs(jd_to_mjd(_jd_utc) - jd_to_mjd(_jd_iso)), abs(OBS_UTC_OFFSET/24.0), rel_tol=TEST_TOLERANCE['3dp'])
...
def test_mjd_to_mjd_2():
    ...
    assert math.isclose(
        abs(mjd_to_jd(_mjd_utc) - mjd_to_jd(_mjd_iso)), abs(OBS_UTC_OFFSET/24.0), rel_tol=TEST_TOLERANCE['3dp'])
...
# -
def test_nearest_0():
    assert all(nearest(_k) is None for _k in TEST_INVALID_INPUTS)

...
def test_non_sidereal_1():
    """ test NonSidereal() for incorrect input(s) """
    with ptr(Exception):
        NonSidereal(telescope=random.choice([None, get_hash(), {}, [], ()]))


def test_non_sidereal_2():
    """ test NonSidereal() for incorrect input(s) """
    with ptr(Exception):
        NonSidereal(instrument=random.choice([None, get_hash(), {}, [], ()]))


# noinspection PyUnresolvedReferences
def test_non_sidereal_3():
    """ test NonSidereal() for correct input(s) """
    _ins = random.choice(INS__INSTRUMENTS)
    _tel = INS__TELESCOPE[_ins]
    assert isinstance(NonSidereal(_tel, _ins), observations.non_sidereal.NonSidereal) in OBS_TRUE_VALUES


# noinspection PyUnresolvedReferences
def test_non_sidereal_4():
    """ test NonSidereal() for correct input(s) """
    _ins = random.choice(INS__INSTRUMENTS)
    _tel = INS__TELESCOPE[_ins]
    assert isinstance(NonSidereal(_tel, _ins).telescope, telescopes.factory.Telescope) in OBS_TRUE_VALUES


# noinspection PyUnresolvedReferences
def test_non_sidereal_5():
    """ test NonSidereal() for correct input(s) """
    _ins = random.choice(INS__INSTRUMENTS)
    _tel = INS__TELESCOPE[_ins]
    assert isinstance(NonSidereal(_tel, _ins).instrument, instruments.factory.Instrument) in OBS_TRUE_VALUES


def test_non_sidereal_6():
    """ test NonSidereal() returns correct attribute(s) """
    _ins = random.choice(INS__INSTRUMENTS)
    _tel = INS__TELESCOPE[_ins]
    _d = NonSidereal(_tel, _ins)
...
# noinspection PyUnresolvedReferences
def test_non_sidereal_10():
    """ test NonSidereal() returns correct attribute(s) """
    _ins = random.choice(INS__INSTRUMENTS)
    _tel = INS__TELESCOPE[_ins]
    _iso, _jd = NonSidereal(_tel, _ins).calculate()
    assert re.match(OBS_ISO_PATTERN, _iso) is not None


# noinspection PyUnresolvedReferences
def test_non_sidereal_11():
    """ test NonSidereal() returns correct attribute(s) """
    _ins = random.choice(INS__INSTRUMENTS)
    _tel = INS__TELESCOPE[_ins]
    _iso, _jd = NonSidereal(_tel, _ins).calculate()

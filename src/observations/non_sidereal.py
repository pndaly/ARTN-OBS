...
class NonSidereal(object):
    ...
    def calculate(self, begin=get_isot(0, True), end=jd_to_isot(isot_to_jd(get_isot(0, True))+OBS_ONE_HOUR)):
        ...
        self.__dec_min = dec_to_decimal(self.__zenith_dec) - NonSidereal.preferred_dec_angle
        self.__dec_max = dec_to_decimal(self.__zenith_dec) + NonSidereal.preferred_dec_angle
        self.__ra_min = ra_to_decimal(self.__zenith_ra) - NonSidereal.preferred_ra_angle
        self.__ra_max = ra_to_decimal(self.__zenith_ra) + NonSidereal.preferred_ra_angle
            ...
            if self.__telescope.is_observable(self.__begin, f"{_e['ra_hms']} {_e['dec_dms']}"):
                if self.__log:
                    self.__log.debug(f"{_e['object_name']} is up and within "
                                     f"RA={NonSidereal.preferred_ra_angle}{UNI__DEGREE}, "
                                     f"Dec={NonSidereal.preferred_dec_angle}{UNI__DEGREE} of zenith")
        ...
        #     if self.__log:
        #         self.__log.debug(f'filter: {NonSidereal.preferred_filter}, '
        ...
        #         f"[{self.__target_name} filter: {NonSidereal.preferred_filter}, binning: {_t}]")
